  ...
  def save_train (self, state, filename):
    ...
    with open (filename, "w") as f:
      ...
      for n in state.new_clusters:
        for o in state.old_clusters:
          if o.name == n.name:
            if o.truth == "s" and n.truth == "s":
              explain = "STAY"
            elif o.truth == "m" and n.truth == "m":
              explain = "MOVE"
            elif o.truth == "v" and n.truth == "v":
              explain = "NEWVIEW"
            else:
              print o.truth, n.truth
              raise Exception("truth does not match")
            f.write ("is(%s,%s,%s)" % (o.oldName(),n.oldName(),explain))
          else:
            f.write ("is(%s,%s,NOT)" % (o.oldName(),n.oldName()))

      f.write("\n")
      for o in state.old_clusters:
        if o.truth == "d":
          f.write ("disappeared(%s)\n" % o.oldName())
        elif o.truth == "p":
          f.write ("hidden(%s)\n" % o.oldName())
